...
class EC(object):
    ...
    def at(self, x):
        ...
        y, my = func.sqrt(ysq, self.q)
        return func.Coord(x, y), func.Coord(x, my)

    def neg(self, p):
        """negate p
        >>> assert ec.is_valid(ec.neg(p))
        """
        return func.Coord(p.x, -p.y % self.q)
    ...
    def add(self, p1, p2):
        ...
        if p1.x == p2.x:
            ...
            l = (3 * p1.x * p1.x + self.a) * func.inv(2 * p1.y, self.q) % self.q
            pass
        else:
            l = (p2.y - p1.y) * func.inv(p2.x - p1.x, self.q) % self.q
            pass
        x = (l * l - p1.x - p2.x) % self.q
        y = (l * (p1.x - x) - p1.y) % self.q
        return func.Coord(x, y)
